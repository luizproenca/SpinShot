...
import {
  ...
  MUSIC_NONE_ID,
  MusicSelection,
  PRESET_MUSIC_PRIORITY,
} from '../constants/music';

const supabase = getSupabaseClient();

const CLOUDINARY_CLOUD_NAME =
  process.env.EXPO_PUBLIC_CLOUDINARY_CLOUD_NAME || 'spinshot360';

const MUSIC_FETCH_TIMEOUT_MS = 12000;
...
function rowToTrack(row: any): MusicTrack {
  const cloudinaryPublicId = row.cloudinary_public_id ?? '';
  ...
  return {
    ...
    emoji: row.emoji ?? '🎵',
    cloudinaryPublicId,
    previewUrl: cloudinaryPublicId
      ? buildCloudinaryPreviewUrl(cloudinaryPublicId)
      : null,
...
export async function fetchMusicTracks(): Promise<MusicTrack[]> {
  ...
  const { data, error } = await withTimeout(query, MUSIC_FETCH_TIMEOUT_MS);
...
export function getAutoTrack(
  ...
  const priority =
    PRESET_MUSIC_PRIORITY[effect] ?? PRESET_MUSIC_PRIORITY['auto'];

  for (const id of priority) {
    const track = tracks.find((t) => t.id === id);
    if (track && (isPro || !track.isPremium)) {
      return track;
    }
  }
